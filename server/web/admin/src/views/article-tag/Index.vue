...
<template>
    <section class="page-container">
        ...
        <el-table :data="results" highlight-current-row border v-loading="listLoading"
			...
			<el-table-column prop="articleId" label="articleId"></el-table-column>
  ...
  export default {
    ...
    methods: {
      list() {
        ...
        HttpClient.post('/api/admin/article-tag/list', params)
          .then(data => {
      ...
      addSubmit() {
        let me = this
        HttpClient.post('/api/admin/article-tag/create', this.addForm)
          .then(data => {
      ...
      handleEdit(index, row) {
        let me = this
        HttpClient.get('/api/admin/article-tag/' + row.id)
          .then(data => {
      ...
      editSubmit() {
        let me = this
        HttpClient.post('/api/admin/article-tag/update', me.editForm)
          .then(data => {
            me.list()
